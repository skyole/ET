                ...
                case CardsType.Straight:
                    ...
                    if (cards.Count >= deskCards.Length)
                    {
                        for (int i = cards.Count - 1; i >= deskCards.Length - 1; i--)
                        {
                            ...
                            {
                                //搜索连续权重牌
                                Card findCard = cards.Where(card => card.CardWeight == cards[i].CardWeight + j).FirstOrDefault();
                                if (findCard == null)
                                {
                                    isTrue = false;
                                    break;
                ...
                case CardsType.DoubleStraight:
                    ...
                    if (cards.Count >= deskCards.Length)
                    {
                        for (int i = cards.Count - 1; i >= deskCards.Length - 1; i--)
                        {
                            ...
                            {
                                //搜索连续权重牌
                                Card[] findCards = cards.Where(card => card.CardWeight == cards[i].CardWeight + (j / 2)).ToArray();
                ...
                case CardsType.TripleStraight:
                    if (cards.Count >= deskCards.Length)
                    {
                        for (int i = cards.Count - 1; i >= deskCards.Length - 1; i--)
                        {
                            ...
                            {
                                //搜索连续权重牌
                                Card[] findCards = cards.Where(card => card.CardWeight == cards[i].CardWeight + (j / 3)).ToArray();
